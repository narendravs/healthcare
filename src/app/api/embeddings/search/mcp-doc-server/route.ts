...
async function getConnectedMcpClient(): Promise<McpClient> {
  try {
    ...
    // Direct path pointing to your compiled background target
    //const mcpServerPath = path.resolve("./dist/mcp-doc-server/mcp-server.js");

    sharedMcpTransport = new StdioClientTransport({
      command: "node",
      args: ["./dist/mcp-doc-server/mcp-server.js"],
    });
